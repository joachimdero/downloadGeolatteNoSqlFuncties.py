...
def DownloadJsonOLD(report, outputJson, BasisUrl, opener, start, limit):
    # download data uit featureserver
    import urllib2
    print ('***start download %s***' % report)
    status = 'uitvoeren'
    outputJsonOpen = open(outputJson, 'wb')
    while status == 'uitvoeren':
        Url = '%s/query?fmt=json&start=%s&limit=%s' % (BasisUrl + report, start, limit)
        ...
        outputJson.write(response)
        start += limit
        print ('download van ', start, 'tot', start + limit)
        if "/" not in response or start > 20000 or report == 'GenummerdeWegenWdb':
            status = 'stop: '
            print('stop: "/" not in response or start > 20000 or report == "GenummerdeWegenWdb"')
...
def RefreshDataGdb(fc_table, out_ws):
    ...
    def TruncateTable(out_ws, out_fc_table):
        ...
        countFeatures = int(str(arcpy.GetCount_management(os.path.join(out_ws, out_fc_table))))
        if countFeatures >= 1:
            print ("nog records aanwezig in table. de table wordt met Delete Features leeggemaakt en truncate table wordt opnieuw uitgevoerd, nog %s records aanwezig" % countFeatures)
...
def MaakTabel(row, WsMem, csvFile, out_map_table, dataType):
    ...
    for FieldName in row:
        ...
        'beginverplaatsing', 'beginverpl', 'beginpositie', 'beginposit', 'eindhm', 'eindverplaatsing', 'eindverpl',
